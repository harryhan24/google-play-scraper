...
return function () {      const promises =('.card').get().slice(0, opts.num).map(function (app) {
...
function parseList() {     return('.card').get().map(function (app) {
return parseApp($(app));
});
}
...
let isSaleItem = app.find('.full-price').text();
isSaleItem = (isSaleItem !== "")

const test = 'https://play.google.com' + app.find('a').attr('href');
let score;
if (scoreText) {
